...
**The real failure axis is vagueness, not darkness (added 2026-07-15, from a reviewer-vs-user judgment mismatch).** In the same EP02 pass, the reviewer (Claude) flagged the two notes that invoke the dead directly ("the labels read 'found with victim'", "the people at Porta Nola can confirm") as borderline, and passed the two soft ones — and the user's judgment was exactly inverted: the "borderline" notes were the good ones, the "safe" ones were the problems. The lesson: don't audit these lines for *risk*; audit them for *landing*. Notes that touch real bodies, real museum labels, real gates are on-brand precisely because they're concrete and factually anchored — the voice guide's own ✅ examples all do this ("영수증을 못 내면 감옥에서 시체로 나가야 했죠"). The victim-mocking prohibition is about the jab's TARGET, not about whether the dead appear: dead-as-record/evidence is Orphea's identity; dead-as-punchline-butt is forbidden. Meanwhile abstraction and object-personification ("the plaster was doing what you asked") are the actual failure mode — indirection that feels clever and safe to the writer but doesn't name anything for the reader. When tempted to soften a note, check whether you're trading force for safety the brand never asked for.
